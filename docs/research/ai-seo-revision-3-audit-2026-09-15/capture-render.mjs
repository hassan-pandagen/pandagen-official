...
for (const view of [{ name: 'desktop', width: 1440, height: 1000 }, { name: 'mobile', width: 390, height: 844 }]) {
  ...
  await send('Page.navigate', { url: 'http://127.0.0.1:3006/services/ai-seo' });
  await delay(1700);
  ...
  if (!data.bodyText.includes('What source tracking can and cannot tell you') || !data.bodyText.includes('What a $500 month contains')) throw new Error('Existing build does not contain revision 3');
  measurements.push({name:view.name,...data});
  await capture(view.name + '-hero');
  for (const [name, prefix] of [['scope','What a $500 month'],['additions','Where it goes beyond'],['close','Tell us what you want']]) {
...
const links = await Promise.all(measurements[0].links.map(async href => {
  const response = await fetch(new URL(href,'http://127.0.0.1:3006'),{method:'HEAD'});
