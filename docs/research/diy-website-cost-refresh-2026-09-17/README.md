...
**Visual limitation:** no browser was available through the computer-use browser interface; creating an in-app browser was also unavailable. Desktop/mobile screenshots, visual table layout and a real keyboard walkthrough were not completed. The content, link destinations and metadata were checked in actual production-build HTML. Do not represent that as full browser QA. A temporary local production server was stopped after this check; no external form submission, payment or message was made.
