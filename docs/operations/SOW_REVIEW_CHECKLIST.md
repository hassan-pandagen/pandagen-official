...
- The Austin address is owner-confirmed as mailing-only and must not be described as a registered or physical office address.
...
- Have Wyoming and Minnesota counsel review governing law, arbitration, late fees, non-solicitation, exclusivity, insurance statements, healthcare duties, and enforceability before signature.

## Precise owner decisions still needed

1. Is `MC Patches LLC` the exact active Wyoming entity and is `PandaCodeGen` its registered DBA/trade name? Is every use of `PandaCodeGen LLC` a typo?
...
9. Confirm that the Wyoming-law/video-arbitration provision is intended for this healthcare engagement. Do not copy it into other clients' terms without a project- and jurisdiction-specific decision.
